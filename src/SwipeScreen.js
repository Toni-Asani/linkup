...
export default function SwipeScreen({ user, setScreen }) {
  ...
  const loadCompanies = async () => {
    ...
    if (user) {
      const { data: myCompany } = await supabase
        ...
        .single()
      if (myCompany) {
        query = query.neq('id', myCompany.id)
      }
    }
  ...
  if (current >= companies.length) return (
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'2rem',textAlign:'center',gap:'1rem'}}>
      ...
      <button onClick={() => { setCurrent(0); setCompanies([...companies]) }}
  ...
  return (
    <div style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',padding:'1.5rem 1rem',gap:'1.5rem',userSelect:'none'}}>
      ...
      <div style={{position:'relative',width:'100%',maxWidth:360,height:420}}>
        ...
        <div
          ...
          style={{
            ...
            transform:`translateX(${offset.x}px) translateY(${offset.y * 0.3}px) rotate(${rotate}deg)`,
            transition: dragging ? 'none' : 'transform 0.4s ease',
            cursor:'grab',zIndex:2,overflow:'hidden',
            ...(decision === 'right' ? {transform:'translateX(120%) rotate(20deg)'} : {}),
            ...(decision === 'left' ? {transform:'translateX(-120%) rotate(-20deg)'} : {}),
          }}
        >
          {/* Header coloré */}
      ...
      {/* Boutons */}
      {!user ? (
        <div style={{background:'#FFF5F5',border:'1px solid #FECACA',borderRadius:12,padding:'1rem',width:'100%',textAlign:'center'}}>
          <p style={{fontSize:13,color:'#E24B4A',fontWeight:600,marginBottom:8}}>🔒 Connectez-vous pour matcher</p>
          <button onClick={() => setScreen('login')}
            style={{padding:'10px 24px',background:'#E24B4A',color:'white',border:'none',borderRadius:10,fontSize:14,fontWeight:600,cursor:'pointer'}}>
            Se connecter
          </button>
        </div>
      ) : (
