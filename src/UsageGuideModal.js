...
export default function UsageGuideModal({ t, onClose }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(15,23,42,0.48)',zIndex:50000,display:'flex',alignItems:'flex-end',justifyContent:'center',padding:'calc(env(safe-area-inset-top) + 0.75rem) 0 0'}} onClick={onClose}>
      <div style={{width:'100%',maxWidth:430,maxHeight:'92dvh',background:'white',borderRadius:'22px 22px 0 0',boxShadow:'0 -18px 55px rgba(15,23,42,0.22)',display:'flex',flexDirection:'column',overflow:'hidden'}} onClick={event => event.stopPropagation()}>
        ...
        <div style={{padding:'1rem 1.25rem calc(env(safe-area-inset-bottom) + 1.25rem)',overflowY:'auto',WebkitOverflowScrolling:'touch'}}>
